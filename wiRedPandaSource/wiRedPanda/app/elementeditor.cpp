...
void ElementEditor::setCurrentElements( const QVector< GraphicElement* > &elms ) {
  m_elements = elms;
  hasLabel = hasColors= hasTone = hasFrequency = canChangeInputSize = hasTrigger = false;
  hasRotation = hasSameLabel = hasSameColors =hasSameTone= hasSameFrequency = false;
  hasSameInputSize = hasSameTrigger = canMorph = hasSameType = hasAnyProperty = false;
  hasElements = false;
  if( !elms.isEmpty( ) ) {
    hasAnyProperty = false;
    hasLabel = hasColors =hasTone= hasFrequency = canChangeInputSize = hasTrigger = true;
    hasRotation = true;
    setVisible( true );
    setEnabled( false );
    int minimum = 0, maximum = 100000000;
    hasSameLabel = hasSameColors = hasSameFrequency = hasSameTone = true;
    ...
    for( GraphicElement *elm : m_elements ) {
      ...
      hasSameTone &= elm->getTone( ) == firstElement->getTone( );
    ...
    /* Tone(Pitch) */
    ui->label_tone->setVisible( hasTone );
    ui->lineEditElementtone->setVisible( hasTone );
    if( hasTone ) {
      if( hasSameTone ) {
        ui->lineEditElementtone->setText( firstElement->getTone( ));
      }
      else {
        ui->lineEditElementtone->setText( _manyTones );
      }
    }
    /* Frequency */
...
void ElementEditor::apply( ) {
  ...
  for( GraphicElement *elm : m_elements ) {
    ...
    if( elm->hasTone( ) && ( ui->lineEditElementtone->text( ) != _manyTones ) ) {
      elm->setTone( ui->lineEditElementtone->text( ) );
    }
...
void ElementEditor::on_lineEditElementtone_editingFinished()
{
  apply( );
}
