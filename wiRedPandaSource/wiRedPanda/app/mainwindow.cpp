...
void MainWindow::populateLeftMenu( ) {
  populateMenu( ui->verticalSpacer_InOut, "VCC,GND,BUTTON,SWITCH,CLOCK,LED,BEEP,DISPLAY", ui->scrollAreaWidgetContents_InOut->layout( ) );
...
void MainWindow::on_actionAbrir_TXT_triggered()
{

    QString path = QFileDialog::getOpenFileName(this,
                                                tr("Open TXT File"),defaultDirectory.absolutePath( ), tr("TXT Files (*.txt)"));
    if(path.isEmpty())
        return;
    QFile file(path);
    ...
    InputButton *reset = new InputButton( );
    Clock *bpm = new Clock();
    InputGnd *gnd=new InputGnd();
    InputVcc *vcc=new InputVcc();

    editor->getScene( )->addItem( reset );
    editor->getScene( )->addItem( bpm);
    editor->getScene( )->addItem( gnd);
    editor->getScene( )->addItem( vcc);
    ...
    while(!in.atEnd()) {
        ...
        if(i==0){
          if(line.toDouble())
             bpm->setFrequency (line.toDouble());
        }else{
          Led *led= new Led();
          cin=NULL;
          QNEConnection *basecon=new QNEConnection();
          editor->getScene( )->addItem( basecon);
          editor->getScene( )->addItem( led );
          led->moveBy(0,(50*i));
          basecon->setPort2(led->input());
          for(int j=line.length()-1;j>=0;j--){
             QNEConnection *cval=new QNEConnection();
             QNEConnection *cclock=new QNEConnection();
             QNEConnection *creset=new QNEConnection();
            //valor;clock;cin;reset

             Box *mcell= new Box(editor);
             editor->getScene( )->addItem( cval );

             editor->getScene( )->addItem( cclock );
             editor->getScene( )->addItem( creset );
             mcell->loadFile(":/boxes/memorycell.panda");

             editor->getScene( )->addItem( mcell );

             cval->setPort2(mcell->input(0));
             cclock->setPort1(bpm->output());
             cclock->setPort2(mcell->input(1));
             creset->setPort1(reset->output());
             creset->setPort2(mcell->input(3));
             if(j==0)
                basecon->setPort1(mcell->output(0));
             if(line[j]=='0')
                    cval->setPort1(gnd->output());
             else
                 cval->setPort1(vcc->output());
             if(cin!=NULL){
                 QNEConnection *ccin=new QNEConnection();
                 editor->getScene( )->addItem( ccin );
                 ccin->setPort1(cin->output(0));
                 ccin->setPort2(mcell->input(2));
